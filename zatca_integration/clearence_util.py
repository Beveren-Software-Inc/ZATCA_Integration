...
def generate_einvoice(doc, method):
    ...
    # TODO Invoice Delivery Date
    invoiceDeliveryDate = (datetime.date.today() + datetime.timedelta(days=10)).strftime("%Y-%m-%d")

    # print("####### Doc START #######")
    # print(json.dumps(doc.as_dict()))
    # print("####### Doc END #######")

    # for item in doc.items:
    #     print("####### Item START #######")
    #     print(json.dumps(item.as_dict()))
    #     print("####### Item END #######")

    tax_percentage =  15 #TODO From Tax Template

    # Line Items
    line_items = []
    for item in doc.items:
        taxableAmount = item.base_amount
        taxAmount = taxableAmount * tax_percentage / 100
    ...
    # Post Invoice Request to Zatca for Clearance
    headers = {

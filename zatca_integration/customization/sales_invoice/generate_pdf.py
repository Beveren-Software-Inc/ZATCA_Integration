...
def _draw_header_section(c, invoice_doc, width, height, margin_x, y, font_name):
    ...
    supply_date = (
        frappe.utils.get_datetime(
            frappe.db.get_value("Delivery Note", invoice_doc.items[0].delivery_note, "posting_date")
        ).strftime("%d-%m-%Y")
        or "-"
    )
    ...
    rows_data = [
        [
            ...
            ":الفاتورة رقم ",
            "Issue Date:",
            str(invoice_doc.posting_date),
            "تاريخ إصدار الفاتورة",
        ],
        [
            "ZATCA Status",
            getattr(invoice_doc, "custom_zatca_submit_status", ""),
            "حالة التخليص",
            "Due Date:",
            str(invoice_doc.due_date or ""),
            "تاريخ الاستحقاق",
        ],
        [
            "Delivery Note:",
...
def _draw_seller_buyer_section(c, invoice_doc, width, margin_x, y, font_name):
    ...
    company_address_line1_arabic = safe_get_value(
        "Address", invoice_doc.company_address, "address_line_1_in_arabic"
    )
    company_address_line2 = safe_get_value("Address", invoice_doc.company_address, "address_line2")
    company_address_line2_arabic = safe_get_value(
        "Address", invoice_doc.company_address, "address_line_2_in_arabic"
    )
    ...
    company_country_arabic = safe_get_value(
        "Address", invoice_doc.company_address, "county_in_arabic"
    )
    company_pincode = safe_get_value("Address", invoice_doc.company_address, "pincode")
    company_additional_number = safe_get_value(
        "Address", invoice_doc.company_address, "additional_number"
    )
    ...
    customer_name_arabic = (
        safe_get_value("Sales Invoice", invoice_doc.name, "customer_name_in_arabic")
        or invoice_doc.customer_name
    )
    customer_cr = (
        safe_get_value("Customer", invoice_doc.customer, "cr") if invoice_doc.customer else "-"
    )

    customer_address_title = safe_get_value(
        "Address", invoice_doc.customer_address, "address_title"
    )
    customer_address_line1 = safe_get_value(
        "Address", invoice_doc.customer_address, "address_line1"
    )
    customer_address_line1_arabic = safe_get_value(
        "Address", invoice_doc.customer_address, "address_line_1_in_arabic"
    )
    customer_address_line2 = safe_get_value(
        "Address", invoice_doc.customer_address, "address_line2"
    )
    customer_address_line2_arabic = safe_get_value(
        "Address", invoice_doc.customer_address, "address_line_2_in_arabic"
    )
    ...
    customer_country_arabic = safe_get_value(
        "Address", invoice_doc.customer_address, "county_in_arabic"
    )
    customer_pincode = safe_get_value("Address", invoice_doc.customer_address, "pincode")
    customer_additional_number = safe_get_value(
        "Address", invoice_doc.customer_address, "additional_number"
    )
    ...
    for i, (label, seller_val, seller_val_arabic, arabic_label) in enumerate(details_data):
        ...
        content_widths = [
            label_width - 10,
            value_width - 10,
            arabic_value_width - 10,
            arabic_label_width - 10,
        ]
        contents = [label, str(seller_val), str(seller_val_arabic), arabic_label]

        for content, content_width in zip(contents, content_widths, strict=False):
        ...
        _draw_table_cell_with_wrapping(
            c, margin_x, current_y, label_width, max_height, label, font_name, 7
        )
        _draw_table_cell_with_wrapping(
            c,
        ...
        _draw_table_cell_with_wrapping(
            c,
            margin_x + label_width + value_width,
            current_y,
            arabic_value_width,
            max_height,
            str(seller_val_arabic),
            font_name,
            7,
            "right",
        )
        _draw_table_cell_with_wrapping(
            c,
            margin_x + label_width + value_width + arabic_value_width,
            current_y,
            arabic_label_width,
            max_height,
            arabic_label,
    ...
    _draw_table_cell_with_wrapping(
        c,
        buyer_start_x + buyer_table_width / 2,
        current_y,
        buyer_table_width / 2,
        base_cell_height,
        ":العميل",
    ...
    for i, (label, buyer_val, buyer_val_arabic, arabic_label) in enumerate(buyer_details_data):
        ...
        _draw_table_cell_with_wrapping(
            c, buyer_start_x, buyer_current_y, label_width, max_height, label, font_name, 7
        )
        _draw_table_cell_with_wrapping(
            c,
            buyer_start_x + label_width,
            buyer_current_y,
            value_width,
            max_height,
            str(buyer_val),
        ...
        _draw_table_cell_with_wrapping(
            c,
            buyer_start_x + label_width + value_width,
            buyer_current_y,
            arabic_value_width,
            max_height,
            str(buyer_val_arabic),
            font_name,
            7,
            "right",
        )
        _draw_table_cell_with_wrapping(
            c,
            buyer_start_x + label_width + value_width + arabic_value_width,
            buyer_current_y,
            arabic_label_width,
            max_height,
            arabic_label,

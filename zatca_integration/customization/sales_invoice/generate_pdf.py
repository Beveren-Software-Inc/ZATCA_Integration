...
invoice_image = str(font_dir / "invoice.jpg")
testing_pdf = str(font_dir / "testing.pdf")
...
def draw_pdf_with_reportlab(temp_pdf_path: str, invoice_doc):
    """Draw Sales Invoice using a PDF converted to image as background."""
    ...
    html = frappe.get_print(
        ...
        print_format="REPC ZATCA SALES INVOICE - PO ITEM - USD BANK DETAILS – V9",  # Use the selected print format
        no_letterhead=1,  # Use letterhead only if specified
        letterhead="REPC New",  # Specify the letterhead if provided
    )
    ...
    # Save to temp file (fitz.open does not accept bytes directly in older versions)
    temp_pdf_path = tempfile.mktemp(suffix=".pdf")
    with open(temp_pdf_path, "wb") as f:
        f.write(pdf_content)
    try:
        doc = fitz.open(testing_pdf)
        page = doc[0]  # first page
        pix = page.get_pixmap(matrix=fitz.Matrix(1.5, 1.5))  # 150 DPI
        temp_image_path = tempfile.mktemp(suffix='.png')
        pix.save(temp_image_path)
        c.drawImage(temp_image_path, 0, 0, width=width, height=height, preserveAspectRatio=False)
        os.remove(temp_image_path)
        doc.close()
        
    except Exception as e:
        frappe.log_error(f"Error converting PDF to image: {e}", "PDF Generator")
        # Fallback: draw simple text if conversion fails
        c.setFont("Helvetica", 12)
        c.drawString(50, height - 50, f"Invoice: {invoice_doc.name}")
        c.drawString(50, height - 80, f"Error loading background PDF")

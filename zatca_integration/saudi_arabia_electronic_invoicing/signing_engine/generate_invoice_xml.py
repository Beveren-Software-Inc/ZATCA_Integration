...
def billing_reference_for_credit_and_debit_note(invoice, sales_invoice_doc):
    ...
    try:
        ...
        frappe.throw(
            _(f"Error occurred while adding billing reference for credit/debit note: {str(error)}")
        )
        return None
...
def xml_tags():
    ...
    try:
        ...
        x509certificate.text = "MIID6TCCA5CgAwIBAgITbwAAf8tem6jngr16DwABAAB/yzAKBggqhkjOPQQ"
...
def invoice_typecode_simplified(invoice, sales_invoice_doc):
    ...
    """
    try:
        invoice_type_code = get_invoice_type_code(sales_invoice_doc)
        cbc_invoicetypecode = ET.SubElement(invoice, "cbc:InvoiceTypeCode")

        if sales_invoice_doc.is_return == 0:
            cbc_invoicetypecode.set("name", invoice_type_code)
            cbc_invoicetypecode.text = "388"
...
def doc_reference_compliance(invoice, sales_invoice_doc, compliance_type):
    """
...
def get_pih_for_company(pih_data, company_name):
    ...
    try:
        ...
        frappe.throw(_(f"Error while retrieving PIH of company '{company_name}' for production."))
        return None
    except (KeyError, AttributeError, ValueError) as e:
        frappe.throw(_(f"Error in getting PIH of company '{company_name}' for production: {e}"))
        return None
...
def company_data(invoice, sales_invoice_doc):
    ...
    try:
        ...
        if sales_invoice_doc.custom_is_zatca_test:
            ...
            config = get_zatca_config_test(
                frappe.get_doc("Company", sales_invoice_doc.company), compliance_csid_doc
            )
    ...
    try:
        ...
        if customer_doc.customer_type != "Individual":
            ...
            if address.address_line1:
                ...
                cbc_streetname_1.text = str(address.address_line1)

            if address.address_line2:
                cbc_buildingnumber_1 = ET.SubElement(cac_postaladdress_1, "cbc:BuildingNumber")
                cbc_buildingnumber_1.text = str(address.address_line2)
            ...
            country_code = (frappe.db.get_value("Country", country_name, "code") or "SA").upper()
        ...
        cbc_registrationname_1.text = str(customer_doc.customer_name)
...
def delivery_and_payment_means_for_compliance(invoice, sales_invoice_doc, compliance_type):
    """
...
def add_document_level_discount_with_tax(invoice, sales_invoice_doc):
    ...
    try:
        ...
        frappe.throw(
            _(f"Error occurred while processing allowance charge data without template: {e}")
        )
        return None
    ...
    try:
        ...
        for single_item in sales_invoice_doc.items:
            line_extension_amount = abs(
                round(single_item.amount / (1 + sales_invoice_doc.taxes[0].rate / 100), 2)
            )
        ...
        if (
            ...
            discount_amount = (
                total_line_extension if difference == 0.01 else sales_invoice_doc.discount_amount
            )
            cbc_amount.text = f"{discount_amount:.2f}"
        ...
        cbc_tax_exemption_reason = ET.SubElement(cac_tax_category, "cbc:TaxExemptionReason")
